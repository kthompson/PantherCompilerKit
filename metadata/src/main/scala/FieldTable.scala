...
case class FieldTable() {
  ...
  def read(buffer: IntList, offset: int): int = {
    ...
    // Four ints per record, matching `write`. It was three, which both strode
    // through the table wrong and read one past the end of the last record;
    ...
    for (i <- 0 to (tableSize - 1)) {
      ...
      val metadataFlags = MetadataFlagsHelpers.fromInt(flags)
      if (metadataFlags == MetadataFlags.Static) {
        staticCount = staticCount + 1
      }
