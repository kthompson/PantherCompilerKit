...
case class Parser(source_file: SourceFile, diagnostics: DiagnosticBag) {
  ...
  def accept(kind: int): SyntaxToken = {
    ...
    if (curr.kind == kind) {
      ...
      diagnostics.reportUnexpectedToken(curr.location, curr.kind, kind)
      new SyntaxToken(source_file, kind, curr.start, "", SyntaxTokenValue.Error, new Array[SyntaxTrivia](0), new Array[SyntaxTrivia](0))
    }
  }

  def is_terminating_line(in_group: bool, expression: Expression): bool = {
  ...
  def hasStatementTerminator(expression: Expression): bool = {
    expression match {
      ...
      case value: BlockExpression => value.closeBrace.is_statement_terminator()
      case value: CallExpression => value.closeParen.is_statement_terminator()
      case value: ForExpression => hasStatementTerminator(value.body)
      case value: GroupExpression => value.closeParen.is_statement_terminator()
      case IdentifierName(value) => value.identifier.is_statement_terminator()
      case value: If => hasStatementTerminator(value.elseExpr)
      case value: IndexExpression => value.closeBracket.is_statement_terminator()
      case value: LiteralExpression => value.token.is_statement_terminator()
      case MatchExpression(_, _, _, _, closeBrace) => closeBrace.is_statement_terminator()
      case value: MemberAccessExpression => value.right.identifier.is_statement_terminator()
      case value: NewExpression => value.closeParen.is_statement_terminator()
      case value: UnaryExpression => hasStatementTerminator(value.expression)
      case value: UnitExpression => value.closeParen.is_statement_terminator()
  ...
  def parse_identifier_name_expression(): Expression = {
  ...
  def parse_group_or_unit_expression(): Expression = {
  ...
  def parse_literal_expression(): Expression = {
  ...
  def parse_new_expression(): Expression = {
  ...
  def parse_for_expression(): Expression = {
    val keyword = accept()
  ...
  def parse_boolean_literal_expression(): Expression = {
    val value = SyntaxTokenValue.Boolean(current_kind() == SyntaxKind.TrueKeyword)
  ...
  def parse_unary_expression(): Expression = {
    ...
    new Expression.UnaryExpression(unary_op, expr)
  }
  ...
  def parse_expression(precedence: int): Expression =
    parse_expression(precedence, false)

  def parse_expression(precedence: int, in_group: bool): Expression = {
  ...
  def parse_call_expression(name: Expression): Expression = {
  ...
  def parse_binary_expression(left: Expression): Expression = {
  ...
  def parse_member_access_expression(left: Expression): Expression = {
    val dot = accept()
  ...
  def parse_index_expression(left: Expression): Expression = {
    val openBracket = accept()
  ...
  def parse_assignment_expression(left: Expression): Expression = {
    val equals = accept()
  ...
  def parse_infix_expression(left: Expression, in_group: bool, precedence: int): Expression = {
  ...
  def parse_match_expression(left: Expression): Expression = {
    val keyword = accept()
    val open = accept(SyntaxKind.OpenBraceToken)
  ...
  def parseEnumCases(): Array[EnumCaseSyntax] = {
    ...
    while (current_kind() != SyntaxKind.EndOfInputToken && current_kind() == SyntaxKind.CaseKeyword) {
