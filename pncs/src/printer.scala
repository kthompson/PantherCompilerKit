...
object AstPrinter {
  ...
  def printExpression(node: Expression): unit = {
    node match {
      case value: Expression.ArrayCreationExpression => printArrayCreationExpression(value)
      case value: Expression.AssignmentExpression => printAssignmentExpression(value)
      ...
      case value: Expression.MemberAccessExpression => printMemberAccessExpression(value)
  ...
  def printArrayCreationExpression(node: Expression.ArrayCreationExpression): unit = {
  ...
  def printMemberAccessExpression(node: Expression.MemberAccessExpression): unit = {
    printExpression(node.left)
  ...
  def printStatment(node: StatementSyntax): unit = {
    node match {
      case value: VariableDeclarationStatement => printVariableDeclarationStatement(value)
  ...
  def printVariableDeclarationStatement(node: StatementSyntax.VariableDeclarationStatement): unit = {
  ...
  def printBreakStatement(node: StatementSyntax.BreakStatement): unit = printToken(node.breakKeyword)

  def printContinueStatement(node: StatementSyntax.ContinueStatement): unit = printToken(node.continueKeyword)

  def printExpressionStatement(node: StatementSyntax.ExpressionStatement): unit = {
    printExpression(node.expression)
  }
  ...
  def _printType(typ: Type, clear: bool): unit = {
    typ match {
      ...
      case Type.Bool =>
        printColor(ColorPalette.Keyword)
        print("bool")
      case Type.Class(ClassName(name)) => 
        printColor(ColorPalette.Identifier)
        print(name)
    }
  ...
  def printSymbolKind(kind: int): unit = {
