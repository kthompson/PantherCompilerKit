...
class TypeMapper(
  ...
  var map: Dictionary[string, Type] =
    buildMap(params, args, DictionaryModule.empty())

  def addMappedType(param: GenericTypeParameter, arg: Type): unit = {
    map = map.put(param.name, arg)
  }

  def buildMap(
      params: List[TypeVariable],
      args: List[Type],
      acc: Dictionary[string, Type]
    ...
    params match {
      case List.Nil =>
        args match {
          ...
          case List.Cons(head, tail) =>
            //            diagnosticBag.reportTypeMismatch(head.location, "Type mismatch")
            ???
            acc
        }
      case List.Cons(param, tailParams) =>
        args match {
          case List.Nil =>
            //            diagnosticBag.reportTypeMismatch(param.location, "Type mismatch")
            ???
            acc
          case List.Cons(arg, tailArgs) =>
            buildMap(tailParams, tailArgs, acc.put(param.name, arg))
        }
    }
  }

  def mapFunction(typ: Type.GenericFunction): Type.Function = {
    val parameters = substituteParameters(typ.parameters)
    val returnType = substitute(typ.returnType)

    Type.Function(typ.location, parameters, returnType)
  }

//  def mapClass(typ: Type.GenericClass): Type.Class = {
//    Type.Class(
//      typ.location,
//      typ.ns,
//      typ.name,
//      substituteList(typ.args),
//      substituteOption(typ.superClass)
//    )
//  }

  def substitute(t: Type): Type = {
    t match {
      case TypeVariable(location, name, _, _) =>
        map.get(name) match {
          case Option.Some(value) => value
          case Option.None =>
            ??? // diagnosticBag.reportTypeMismatch(location, "Type mismatch")
            Type.Error
        }
      ...
      case Type.Class(symbol, args) =>
        Type.Class(symbol, substituteList(args))

      case Type.GenericClass(symbol, genericTypeParams) =>
        Type.GenericClass(
          symbol,
          genericTypeParams // TODO: substituteList(genericTypeParams),
        )
      case Type.Error => t
      case Type.Any   => t
      case Type.Never => t
  ...
  def substituteOption(t: Option[Type]): Option[Type] = {
    t match {
      case Option.Some(value) => Option.Some(substitute(value))
  ...
  def substituteParameters(
    ...
    value match {
      ...
      case List.Cons(head, tail) =>
        List.Cons(
          BoundParameter(head.name, substitute(head.typ)),
          substituteParameters(tail)
        )
    }
  }
}
