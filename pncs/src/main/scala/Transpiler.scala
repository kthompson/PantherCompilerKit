...
case class Transpiler(
  ...
  /** Scala and Panther spell a trait the same way, so this is a straight token
    * walk with no rewriting.
    */
  def transpileTraitDeclaration(
      decl: MemberSyntax.TraitDeclarationSyntax,
      context: TranspilerContext
  ): unit = {
  ...
  /** Always `Option.None` when the input is Scala, which has no `derive`
    * attribute to read — a `case class`'s attribute is supplied by
    * `transpileClassDeclaration` rather than read from the source. This is here
    * so that a `.pn` source round-trips.
    */
  ...
  def transpileTypeAnnotation(
    ...
    transpileTypeName(typeAnnotation.typ, context)
  }

  /** A type name, where Scala's spelling of a builtin differs from Panther's.
    *
  ...
  def transpileTypeName(name: NameSyntax, context: TranspilerContext): unit = {
    name match {
      case NameSyntax.SimpleName(
        ...
        transpileTokenWithText(
          identifier,
          pantherTypeName(identifier.text),
          context
        )
      case _ => transpileName(name, context)
    }
  }

  /** Scala's name for a builtin, as Panther spells it. Anything else is a type
    * the sources declared and is left alone.
    */
  def pantherTypeName(text: string): string = {
    if (text == "String") "string"
    else if (text == "Boolean") "bool"
    else if (text == "Unit") "unit"
    else text
  }
...
//    transpileOptionalToken(parameter.modifier, context)
  ...
  def transpileTypeArgumentList(
    ...
    for (x <- 0 to (typeArgumentlist.arguments.length - 1)) {
      ...
      // `Array[String]` is a type argument, so the same rewrite applies
      transpileTypeName(argument.name, context)
