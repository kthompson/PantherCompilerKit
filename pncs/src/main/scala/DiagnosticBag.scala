...
case class DiagnosticBag(settings: CompilerSettings) {
  ...
  def reportTypeCircularity(location: TextLocation, name: Type): unit =
    report(location, "Circular use of variable in type" + string(name))
  ...
  def reportCannotConvert(
    ...
    report(
      location,
      "Cannot convert from " + string(from) + " to " + string(toType)
    )
  ...
  def reportSymbolNotFoundForType(
    ...
    report(location, "Symbol " + name + " not found for type " + string(left))
  ...
  def reportNoOperatorForOperands(
    ...
    report(
      location,
      "No operator '" + op + "' for operands " + string(left) + " and " +
        string(right)
    )
  ...
  def reportNoOperatorForOperand(
    ...
    report(location, "No operator '" + op + "' for operand " + string(operand))
