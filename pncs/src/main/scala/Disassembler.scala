...
/** Writes into a builder rather than to the console directly, so the same
  * disassembly can be printed as it is produced — which is what tracing does —
  * or accumulated and handed back, which is what anything comparing it wants.
  * `IndentedStringBuilder(true)` is the printing one.
  */
case class Disassembler(
    chunk: Chunk,
    metadata: Metadata,
  ...
  def write(value: string): unit = sb.append(value)
  ...
  def disassembleMethod(token: MethodToken): bool = {
    ...
    if (method.address == -1) {
      ...
      writeLine(metadata.getMethodName(token) + ":")
  ...
  def disassembleInstruction(offset: int): int = {
    // offset
    write(Pad.left(Hex.toString(offset), 4, '0') + " ")
    ...
    if (offset > 0 && chunk.readLine(offset) == chunk.readLine(offset - 1)) {
      ...
      val lineStr = Pad.left(string(line), 4, ' ')
      write(lineStr + " ")
    }
    ...
    if (opcode == Opcode.Ret) {
      ...
      methodTokenInstruction(chunk, offset, name)
    } else if (opcode == Opcode.Calli) {
      // no operand to print: the token is on the stack, not in the stream
      simpleInstruction(name, offset)
    } else if (opcode == Opcode.Callx) {
      builtinInstruction(chunk, offset, name)
  ...
  def fieldTokenInstruction(chunk: Chunk, offset: int, name: string): int = {
    ...
    writeLine(name + " " + fieldName)
    offset + 2
  }

  /** `callx` prints what it calls, the way `call` prints a method name — the
    * operand is a builtin id and the number on its own says nothing.
    */
  def builtinInstruction(chunk: Chunk, offset: int, name: string): int = {
    val id = chunk.readI4(offset + 1)
    writeLine(name + " " + Builtin.nameOf(id))
    offset + 2
  }
