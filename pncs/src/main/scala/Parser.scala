...
//      )
      ...
      if (currentKind() == SyntaxKind.ColonToken) {
        ...
        identFromName(name) match {
          case Option.Some(identifier) =>
            val inner = if (identifier.text == "_") {
              SymbolPatternSyntax.Discard(identifier)
            } else {
              SymbolPatternSyntax.Identifier(identifier)
            }
          ...
          case Option.None =>
            ...
            PatternSyntax.Object(typeAnnotation.typ)
        }
  ...
  def patternFromIdentifier(identifier: SyntaxToken): PatternSyntax = {
    if (identifier.text == "_") {
      PatternSyntax.Variable(SymbolPatternSyntax.Discard(identifier))
    } else {
      PatternSyntax.Variable(SymbolPatternSyntax.Identifier(identifier))
    }
  }
