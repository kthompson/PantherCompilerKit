...
case class Parser(
  ...
  /** The token `offset` places ahead, clamped to the end of input.
    *
    * Reads the array directly rather than going through `next()`, which is
    * correct here because the only tokens `next()` drops are annotations and
    * `override`, and neither can appear in the lookahead this is used for.
    */
  def peek(offset: int): SyntaxToken = {
    val len = _tokens.length
    val index = _position + offset

    if (index >= len) _tokens(len - 1)
    else _tokens(index)
  }
  ...
  def currentPrecedence(): int = {
    ...
    else if (kind == SyntaxKind.PlusToken || kind == SyntaxKind.DashToken) 8
    else if (
      kind == SyntaxKind.StarToken || kind == SyntaxKind.SlashToken || kind == SyntaxKind.PercentToken
    ) 9
    // unary/prefix is 10
    else if (
      kind == SyntaxKind.OpenParenToken || kind == SyntaxKind.OpenBracketToken || kind == SyntaxKind.DotToken
    ) 11
  ...
  def nameHasStatementTerminator(name: NameSyntax): bool = {
    name match {
      case NameSyntax.SimpleName(name) => simpleNameHasStatementTerminator(name)
      case NameSyntax.QualifiedName(left, dotToken, right) =>
        simpleNameHasStatementTerminator(right)
    }
  }
  ...
  /** `trait Eq[T] { def equals(a: T, b: T): bool }`
    *
    * A trait takes no constructor parameters and its template is mandatory: an
    * empty capability is a class, not a trait. Members parse through the
    * ordinary member path, so a `def` with no body — which
    * `FunctionDeclarationSyntax` already allows — is how a requirement is
    * spelled.
    */
  def parseTraitDeclaration(): MemberSyntax = {
    debugPrint("parseTraitDeclaration")
    val keyword = acceptKind(SyntaxKind.TraitKeyword)
    val identifier = acceptKind(SyntaxKind.IdentifierToken)
    val typeParams = parseGenericTypeParameters()
    val template = parseTemplate()

    // Qualified: the self-hosted compiler does not resolve a bare enum case
    // name in a `new` position.
    new MemberSyntax.TraitDeclarationSyntax(
      keyword,
      identifier,
      typeParams,
      template
  ...
  /** `given Eq[int] { … }` and `given [T: Ord] => Ord[List[T]] { … }`.
    *
    * The bracketed parameter list, when present, is the premise, and `=>`
    * separates it from the trait head. There is no ambiguity to resolve: an
    * unconditional given starts with the trait's identifier, a conditional one
    * with `[`.
    */
  def parseGivenDeclaration(): MemberSyntax = {
    debugPrint("parseGivenDeclaration")
    val keyword = acceptKind(SyntaxKind.GivenKeyword)
    val typeParams = parseGenericTypeParameters()
    val arrow = typeParams match {
      case Option.None => Option.None
      case Option.Some(_) =>
        Option.Some(acceptKind(SyntaxKind.EqualsGreaterThanToken))
    }
    val name = parseName(false)
    val template = parseTemplate()

    new MemberSyntax.GivenDeclarationSyntax(
      keyword,
      typeParams,
      arrow,
      name,
  ...
  def parseDeriveAttribute(): Option[DeriveAttributeSyntax] = {
    if (currentKind() != SyntaxKind.OpenBracketToken) Option.None
      ...
      debugPrint("parseDeriveAttribute")
      val openBracket = accept()
      val derive = acceptKind(SyntaxKind.IdentifierToken)

      // `derive` is the only attribute there is, and it is contextual like
      // `operator`: reserving it would take a name the sources use.
      if (derive.text != "derive") {
        diagnostics.reportUnknownAttribute(derive.location, derive.text)
      } else ()

      val openParen = acceptKind(SyntaxKind.OpenParenToken)
      val traits = parseDerivedTraits()
      val closeParen = acceptKind(SyntaxKind.CloseParenToken)
      val closeBracket = acceptKind(SyntaxKind.CloseBracketToken)

      Option.Some(
        new DeriveAttributeSyntax(
          openBracket,
          derive,
          openParen,
          traits,
  ...
  def parseDerivedTraits(): List[DerivedTraitSyntax] = {
    var traits: List[DerivedTraitSyntax] = List.Nil
    var more = true
    ...
    while (
      ...
      val name = acceptKind(SyntaxKind.IdentifierToken)
      // annotated: the self-hosted compiler does not widen the branches of an
      // `if` to their common supertype
      val comma: Option[SyntaxToken] =
        if (currentKind() == SyntaxKind.CommaToken) {
          Option.Some(accept())
        } else {
          // the last name in the list; anything else is the caller's `)`
          more = false
          Option.None
        }

      traits = List.Cons(new DerivedTraitSyntax(name, comma), traits)
    }

    ListModule.reverse(traits)
  }
...
//      )
  ...
  def parsePattern(): PatternSyntax = {
    ...
    if (
      ...
      currentKind() == SyntaxKind.CharToken || currentKind() == SyntaxKind.TrueKeyword ||
      currentKind() == SyntaxKind.FalseKeyword
    ) {
      val token = accept()
      PatternSyntax.Literal(token)
    } else {
      val name = parseName(false)
      if (currentKind() == SyntaxKind.ColonToken) {
        ...
        identFromName(name) match {
          ...
          case Option.None =>
            ...
            PatternSyntax.Type(typeAnnotation.typ)
        }
  ...
  def parseExtractPattern(nameSyntax: NameSyntax): PatternSyntax = {
    ...
    if (currentKind() == SyntaxKind.CloseParenToken) {
      // empty pattern list
      val close = accept()
      PatternSyntax.Extract(
        nameSyntax,
        open,
        new Array[PatternItemSyntax](0),
        close
      )
    } else {
      val patterns = parsePatternList()
      val close = acceptKind(SyntaxKind.CloseParenToken)
      PatternSyntax.Extract(nameSyntax, open, patterns, close)
    }
  }
  ...
  /** Whether the member starting here is `operator ==(a: T, b: T): bool`.
    *
  ...
  def isOperatorDeclaration(): bool =
    current().kind == SyntaxKind.IdentifierToken &&
      ...
      SyntaxFacts.isOperatorKind(peek(1).kind) &&
  ...
  def parseOperatorDeclaration(): MemberSyntax = {
    ...
    val operatorKeyword = accept()
    val identifier = accept()

    // Only a token the language already has may be declared. `operator ~`
    // reaches here because `~` is an operator token, but nothing parses `a ~ b`
    // as a binary expression, so a trait claiming it could never be reached.
    if (!SyntaxFacts.isBinaryOperator(identifier.kind)) {
      diagnostics.reportNotABinaryOperator(identifier.location, identifier.text)
    } else ()
  ...
  def parseMember(topLevelStatement: bool): MemberSyntax = {
    ...
    // Only a class or an enum can be derived for, so an attribute on anything
    // else is reported where the member is parsed rather than here — the
    ...
    if (kind == SyntaxKind.ObjectKeyword) {
      rejectDerives(derives, "an object")
      parseObjectDeclaration()
    } else if (
      kind == SyntaxKind.CaseKeyword && scala ||
      kind == SyntaxKind.ClassKeyword
    ) {
      parseClassDeclaration(derives)
    } else if (kind == SyntaxKind.DefKeyword) {
      rejectDerives(derives, "a function")
      parseFunctionDeclaration()
    } else if (kind == SyntaxKind.EnumKeyword) {
      parseEnumDeclaration(derives)
    } else if (kind == SyntaxKind.TraitKeyword) {
      rejectDerives(derives, "a trait")
      parseTraitDeclaration()
    } else if (kind == SyntaxKind.GivenKeyword) {
      rejectDerives(derives, "a given")
      parseGivenDeclaration()
    } else if (isOperatorDeclaration()) {
      rejectDerives(derives, "an operator")
      parseOperatorDeclaration()
    } else if (kind == SyntaxKind.ValKeyword || kind == SyntaxKind.VarKeyword) {
      rejectDerives(derives, "a variable")
      parseVariableDeclaration()
    } else {
      rejectDerives(derives, "a statement")
      parseGlobalStatement()
    }
  }

  /** A `derive` attribute belongs on a class or an enum: those are the only
    * declarations with constructor parameters to derive over.
    */
  def rejectDerives(
      derives: Option[DeriveAttributeSyntax],
      what: string
    ...
    derives match {
      ...
      case Option.Some(attribute) =>
        diagnostics.reportDeriveNotSupported(
          attribute.deriveToken.location,
          what
        )
    }
  }
