...
class AstPrinter(withColor: bool, buffer: IndentedStringBuilder) {
  ...
  def printGivenDeclaration(node: MemberSyntax.GivenDeclarationSyntax): unit = {
    printTokenWithColor(node.givenKeyword, ColorPalette.Keyword)
    node.genericParameters match {
      case Option.None        => ()
      case Option.Some(value) => printGenericParameters(value)
    }
  ...
  def printExpression(node: Expression): unit = {
    node match {
      ...
      case Expression.IdentifierName(value) => printSimpleName(value)
      ...
      case value: Expression.Match => panic("MatchExpression not implemented")
  ...
  def printStatment(node: StatementSyntax): unit = {
    node match {
      ...
      case value: StatementSyntax.BreakStatement => printBreakStatement(value)
      case value: StatementSyntax.ContinueStatement =>
        printContinueStatement(value)
      case value: StatementSyntax.ExpressionStatement =>
        printExpressionStatement(value)
    }
  }
