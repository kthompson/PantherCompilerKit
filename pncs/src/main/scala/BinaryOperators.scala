...
case class BinaryOperators(binder: Binder) {
  ...
  // Char arithmetic answers in int. `c - '0'` is how a digit character
  // becomes its value and `'A' + n` is how one is built, and a char is an int
  // at runtime anyway, so there is no representation to preserve by answering
  // in char — the caller writes `char(…)` when it wants one back.
  binaryOp(
    binder.charType,
    binder.charType,
