...
//    panic("the type above probably should include the symbols we started to type, as well as all the parameters for methods")
  ...
  def registerEnumDerivationList(
    ...
    traits match {
      case List.Nil => ()
      case List.Cons(head, tail) =>
        registerEnumDerivation(enumSymbol, head.name, cases, isGeneric)
        registerEnumDerivationList(enumSymbol, tail, cases, isGeneric)
    }
  }
  ...
  /** The enum half of `registerDerivation`.
    ...
    * carries `List.Nil` for its arguments whether or not it is generic, so
    * whether it has type parameters has to be passed in rather than read back
    * off the type.
    */
