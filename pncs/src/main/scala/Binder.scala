...
/** A resolved proof, and the static field its record lives in.
  ...
  * proves a different thing at every instantiation
  * ([ADR 0006](../../../docs/architecture/adr/0006-conditional-givens.md),
  * decision B). `premises` are the given's constraints with this
  * instantiation's type arguments substituted in, so they are ground goals too
  * and name the records the dependency half points at — `Eq[List[Symbol]]`
  * holding a reference to `Eq[Symbol]`, which is ADR 0005's recursive evidence.
  */
...
case class BoundGiven(
  ...
  /** Everything the record's dependency half holds, in slot order: the declared
    ...
    * instantiation exactly as a premise is
    * ([ADR 0006](../../../docs/architecture/adr/0006-conditional-givens.md)).
    *
...
/** One case of an enum being derived for, in declaration order.
  ...
  * a case per alternative and no parameters of its own. The order is the
  * enum's own, which is what `Ord` compares by before it looks at any
  * parameter
  * ([ADR 0004](../../../docs/architecture/adr/0004-traits-given-evidence-and-contextual-extensions.md)).
  */
...
//  )
  ...
  /** `system.io`, which the Scala side shims in
    ...
    * one file `sbt pncs/transpile` skips ([`build.sbt:43`](../../../../build.sbt:43)),
    * so there is no generated `.pn` for these to bind to and they have to be
    * declared here like the rest of the prelude.
    *
  ...
  /** Every evidence record the program needs, in reverse discovery order.
    ...
    * per given: a conditional given has no single record to lay out
    * ([ADR 0006](../../../docs/architecture/adr/0006-conditional-givens.md),
    * decision B). Discovery order is a function of the source, which is what
    * stage 3 needs — the field order is part of the bytecode.
    *
...
//    panic("the type above probably should include the symbols we started to type, as well as all the parameters for methods")
  ...
  /** Finds the given that proves `goal`, interns a record for it, and does the
    ...
    * is allocated but not yet filled is still a usable reference
    * ([ADR 0006](../../../docs/architecture/adr/0006-conditional-givens.md),
    * decision E).
    */
  def resolveEvidence(
  ...
  def dependencySlot(candidate: BoundGiven, goal: Type): int = {
    ...
    indexOfConstraint(candidate.dependencies, goal, 0) match {
      ...
      case Option.None =>
        ...
        candidate.dependencies =
          appendConstraint(candidate.dependencies, goal)
        base + index
    }
  }
  ...
  /** The enum `typ` is a case of, carrying the case's own type arguments.
    ...
    * instantiate it. A goal has to keep the arguments —
    * `Eq[List.Cons<int>]` widens to `Eq[List<int>]`, not to `Eq[List]` — and a
    * case's type parameters are the enum's, in order, because `bindEnumCases`
