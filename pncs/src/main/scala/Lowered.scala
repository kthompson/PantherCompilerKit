...
case class LoweredLeftHandSideBlock(
  ...
  def filterUnitStatementsChain(
    ...
    block.uncons() match {
      ...
      case Option.Some(Tuple2(statement, rest)) =>
        statement match {
          case LoweredStatement.ExpressionStatement(
                _,
                LoweredExpression.Unit
              ) =>
            // skip this statement
            filterUnitStatementsChain(rest)
          case _ =>
            // keep this statement
            ChainModule.of(statement).concat(filterUnitStatementsChain(rest))
        }
    }
  }
...
class ExpressionLowerer(symbol: Symbol, binder: Binder) {
  ...
  def lhsToExpression(expression: LoweredLeftHandSide) = {
    expression match {
      ...
      case LoweredLeftHandSide.New(location, constructor, genericArgs, args) =>
...
//            .concat(condGoto)
  ...
  def lowerLeftHandSide(
    ...
    lhs match {
      ...
      case BoundLeftHandSide.Variable(location, symbol) =>
        lowerVariable(location, symbol, context)
    }
  }
