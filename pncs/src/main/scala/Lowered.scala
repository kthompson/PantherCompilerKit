...
//            .concat(condGoto)
      ...
      case Option.None =>
        ...
        // there is no else so we dont need a variable assignment as this is a unit expression
        checkUnusedExpr(thenBlock)
  ...
  def lowerMemberAccess(
    ...
    val lowered = lowerExpression(expr.left, context)
    LoweredBlock(
      lowered.statements,
