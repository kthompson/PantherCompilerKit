...
case class Emitter(
  ...
  /** A builtin member has no body to call into, so each one is its own opcode.
    *
    * The receiver and then the arguments are already on the stack, which is the
    * order these opcodes pop them in.
    */
  def emitExternCall(method: Symbol, line: int): unit = {
    if (method.name == "apply") {
      method.parent.get().name match {
        case "string" =>
          chunk.emitOpcode(Opcode.ConvStr, line)
        case "bool" =>
          chunk.emitOpcode(Opcode.ConvBool, line)
        case "int" =>
          chunk.emitOpcode(Opcode.ConvI4, line)
        case "char" =>
          chunk.emitOpcode(Opcode.ConvChar, line)
        case _ =>
          panic("emitExternCall: unknown extern " + method.name)
      }
    } else if (method.name == "substring") {
      chunk.emitOpcode(Opcode.Substr, line)
    } else if (method.name == "endsWith") {
      chunk.emitOpcode(Opcode.EndsWith, line)
    } else if (method.name == "compareTo") {
