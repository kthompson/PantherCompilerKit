...
case class Emitter(
  ...
  def emitAssignmentStatement(
    ...
    if (expr.variable.kind == SymbolKind.Field) {
      fieldTokens.get(expr.variable) match {
        case Option.Some(value) =>
          assert(expr.variable.isStatic(), "expected static field")
          val op = Opcode.Stsfld
          chunk.emitOpcode(op, expr.location.startLine)
          chunk.emitI4(value.token, expr.location.startLine)
        case Option.None =>
          panic("emitAssignmentStatement: no field token for " + expr.variable)
      }
    } else if (expr.variable.kind == SymbolKind.Local) {
      ...
      panic(
        "emitAssignmentStatement: unsupported symbol kind " + expr.variable.kind
      )
    }
  }
