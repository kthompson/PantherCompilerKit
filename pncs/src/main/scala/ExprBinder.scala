import BoundLeftHandSide.IndexExpression
import panther.*
      ...
      case node: Expression.IdentifierName =>
        bindIdentifierName(node, scope) match {
          ...
          case Result.Success(value) =>
            Result.Success(BoundLeftHandSide.Variable(value.symbol))
        }
      case node: Expression.MemberAccessExpression =>
        bindMemberAccessExpression(node, scope) match {
          case Result.Error(value) => Result.Error(value)
...
//  }
  ...
  def getLHSType(node: BoundLeftHandSide): Type = {
    node match {
      ...
      case BoundLeftHandSide.IndexExpression(indexExpression) =>
