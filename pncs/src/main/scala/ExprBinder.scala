...
case class ExprBinder(
  ...
  /** Whether a call's target is a trait member reached through evidence rather
    * than an ordinary method. Only `bindMemberForSymbolAndType` produces one,
    * and only where the receiver's type has no members of its own.
    */
  def isTraitMemberAccess(function: BoundLeftHandSide): bool = {
    function match {
      case BoundLeftHandSide.MemberAccess(access) =>
        access.member.parent match {
          case Option.Some(owner) => owner.kind == SymbolKind.Trait
            ...
            binder.tryGetSymbolType(member) match {
              case Option.Some(Type.Function(_, parameters, returnType)) =>
                if (parameters.length != allArgs.length) {
                  diagnosticBag.reportArgumentCountMismatch(
                    location,
                    // the receiver is one of them, so report what the user
                    // has to write rather than the elaborated count
                    parameters.length - 1,
                    args.length
                  )
                  Result.Error(
  ...
  def bindPattern(
    ...
    pattern match {
      ...
      case PatternSyntax.Type(_) =>
        // A bare type pattern binds nothing; the check is the lowerer's
        Result.Success(BoundPattern.Discard)
      case PatternSyntax.TypeAssertion(innerPattern, typeAnnotation) =>
        ...
        binder.bindTypeName(typeAnnotation.typ, scope) match {
          case Type.Error(message) =>
            Result.Error(BoundExpression.Error(message))
          case annotatedType =>
            bindPattern(innerPattern, scope, annotatedType)
        }
