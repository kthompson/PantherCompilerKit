...
case class ExpressionItemSyntax(
...
enum Expression {
  ...
  case Literal(token: SyntaxToken, value: SyntaxTokenValue)
  case MemberAccess(
      left: Expression,
      dotToken: SyntaxToken,
      right: SimpleNameSyntax
  ...
  case Unary(operator: SyntaxToken, expression: Expression)
  case Unit(openParen: SyntaxToken, closeParen: SyntaxToken)
  case While(
...
enum PatternSyntax {
  ...
  case Variable(value: SymbolPatternSyntax)
  case Object(typ: NameSyntax)
  case TypeAssertion(
      value: SymbolPatternSyntax,
      typeAnnotation: TypeAnnotationSyntax
  )
  case Literal(value: SyntaxToken)
}

case class PatternItemSyntax(
    pattern: PatternSyntax,
    separatorToken: Option[SyntaxToken]
)
...
case class TypeArgumentItemSyntax(
    name: NameSyntax,
...
case class EnumCaseParametersSyntax(
    openParenToken: SyntaxToken,
    parameters: List[ParameterSyntax],
    closeParenToken: SyntaxToken
)
case class EnumCaseSyntax(
    caseKeyword: SyntaxToken,
    identifier: SyntaxToken,
...
case class GenericParameterSyntax(
    variance: Option[SyntaxToken],
    identifier: SyntaxToken,
    bounds: Option[GenericBoundsSyntax]
)
case class GenericParametersSyntax(
    lessThanToken: SyntaxToken,
...
/** names [1..n]
  */
case class NamespaceDeclarationSyntax(
...
/** namespaceDeclaration [0..1] usings [0..n] members [0..n]
  */
case class CompilationUnitSyntax(
...
object AstUtils {
  ...
  def locationOfExpression(value: Expression): TextLocation = {
    value match {
      ...
      case Expression.Is(expression, isKeyword, typ) =>
        locationOfExpression(expression).merge(locationOfName(typ))
      case Expression.Literal(token, value) =>
        token.location
      case Expression.MemberAccess(left, dotToken, right) =>
        locationOfExpression(left).merge(locationOfSimpleName(right))
      case Expression.Match(expression, _, _, _, closeBrace) =>
        locationOfExpression(expression).merge(closeBrace.location)
      case Expression.New(newKeyword, _, _, _, closeParen) =>
        newKeyword.location.merge(closeParen.location)
      case Expression.Unary(operator, expression) =>
        operator.location.merge(locationOfExpression(expression))
      case Expression.Unit(openParen, closeParen) =>
        openParen.location.merge(closeParen.location)
      case Expression.While(whileKeyword, _, _, _, body) =>
  ...
  def locationOfBoundExpression(value: BoundExpression): TextLocation = {
    value match {
      ...
      case BoundExpression.Binary(location, left, _, right, _) =>
      ...
      case BoundExpression.Index(location, _, _, _)           => location
      case BoundExpression.Match(location, _, _, _)           => location
      case BoundExpression.Int(location, _)                   => location
      case BoundExpression.Is(location, _, _)                 => location
      case BoundExpression.MemberAccess(location, _, _, _, _) => location
      case BoundExpression.New(location, _, _, _, _)          => location
      case BoundExpression.String(location, _)                => location
      case BoundExpression.Unary(location, _, operand, _) =>
        location.merge(locationOfBoundExpression(operand))
      case BoundExpression.Unit(location)           => location
      case BoundExpression.Variable(location, _, _) => location
      case BoundExpression.While(location, _, _)    => location
