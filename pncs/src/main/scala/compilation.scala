...
case class Compilation(
  ...
  /** The printers, as text.
    ...
    * snapshot of a coloured tree is a snapshot of the escape codes. The builder
    * accumulates rather than printing as it goes, which is the other half of
    * being able to hand the result back.
    */
...
object ImageRunner {
  ...
  /** Runs a command-line image whose entry point accepts `Array[string]`. */
  def execWithArgs(
      path: string,
      settings: CompilerSettings,
      args: Array[string]
