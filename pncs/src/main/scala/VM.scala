...
case class VM(
  ...
  def performCast(value: Value, targetTypeToken: TypeDefToken): Value = {
    ...
    // Subsuming an expression to unit discards its value. This is how a block
    // treats every non-final expression, and is valid for every source type.
    if (targetTypeName == "unit") {
      Value.Uninitialized
    } else
      value match {
        case Value.Int(i) =>
        ...
        case Value.Uninitialized =>
          targetTypeName match {
            ...
            case _ =>
              runtimeError(
                "Cannot cast uninitialized value to " + targetTypeName
              )
              Value.Uninitialized
          }
      }
  ...
  /** Runs an entry point of the form `main(args: Array[string])`.
    *
    * Images normally start with an empty stack. A command-line program is the
    * exception: its argument array is a host value, so the VM has to lay it out
    * on the heap and push its reference before building the entry frame.
    */
  def runWithArgs(args: Array[string]): InterpretResult = {
    setupHeap()

    entry match {
      case Option.None => runtimeError("Image has no entry point")
      case Option.Some(value) =>
        if (metadata.getMethodParameterCount(value) != 1) {
          runtimeError(
            "Image entry point does not accept command-line arguments"
          )
        } else {
          metadata.findTypeDefByName("string") match {
            case Option.None =>
              runtimeError("Image has no type token for string arguments")
            case Option.Some(stringType) =>
              val address = alloc(args.length + 1)
              heap(address) = Value.Int(args.length)
              for (i <- 0 to (args.length - 1)) {
                heap(address + 1 + i) = Value.String(args(i))
              }
              push(Value.Ref(stringType, address))
              runEntry()
          }
        }
    }
