...
case class NodeLinks() {
  ...
  def hasSymbol(): bool = _symbol.isDefined

  def getSymbol(): Symbol = _symbol.get

  def setSymbol(symbol: Symbol): Symbol = {
    _symbol = Some(symbol)
    symbol
  }
...
case class Checker(diagnosticBag: DiagnosticBag, root: Symbol) {

  var lastSymbolId = 0
  var symbolLinks = new Array[SymbolLinks](10)
  var lastNodeId = 0
  var nodeLinks = new Array[NodeLinks](10)
  var lastTypeId = 0
  var typeLinks = new Array[TypeLinks](10)
...
//  }
  ...
  def checkClasses(symbol: Symbol): unit = {
    if(symbol.kind == SymbolKind.Class) {
      ...
      if(!links.hasType()) {
        links.setType(Type.Reference(symbol, None))
      } else ()
    } else ()
  ...
  def getSymbolId(symbol: Symbol): int = {
    if (symbol.id != -1) {
      ...
      if (lastSymbolId + 1 >= symbolLinks.length) {
        val newItems = new Array[SymbolLinks]((lastSymbolId + 1) * 2)
        for (i <- 0 to (lastSymbolId - 1)) {
          newItems(i) = symbolLinks(i)
        }
        symbolLinks = newItems
      } else {}

      symbolLinks(lastSymbolId) = new SymbolLinks()
      symbol.id = lastSymbolId
      lastSymbolId = lastSymbolId + 1
      symbol.id
    }
  }
...
// one shortcut could be that we assign Type.Any the type `Type` instead of it having `Type.Any`
  ...
  def getTypeOfExpression(expr: Expression, scope: Scope): Type = {
    expr match {
      case value: Expression.ArrayCreationExpression => getTypeOfArrayCreationExpression(value, scope)
      case value: Expression.AssignmentExpression => getTypeOfAssignmentExpression(value, scope)
      case value: Expression.BinaryExpression => getTypeOfBinaryExpression(value, scope)
      case value: Expression.BlockExpression => getTypeOfBlockExpression(value, scope)
      case value: Expression.CallExpression => getTypeOfCallExpression(value, scope)
      ...
      case expr: Expression.MatchExpression => getTypeOfMatchExpression(expr, scope)
      case value: Expression.NewExpression => getTypeOfNewExpression(value, scope)
      case value: Expression.UnaryExpression => getTypeOfUnaryExpression(value, scope)
      case value: Expression.UnitExpression => getTypeOfUnitExpression(value, scope)
      case value: Expression.WhileExpression => getTypeOfWhileExpression(value, scope)
    }
  }

  def getTypeOfArrayCreationExpression(value: Expression.ArrayCreationExpression, scope: Scope): Type = panic("todo")

  def getTypeOfAssignmentExpression(value: Expression.AssignmentExpression, scope: Scope): Type = panic("todo")

  def getTypeOfBinaryExpression(value: Expression.BinaryExpression, scope: Scope): Type = {
  ...
  def getTypeOfCallExpression(value: Expression.CallExpression, scope: Scope): Type = {
  ...
  def getTypeOfForExpression(value: Expression.ForExpression, scope: Scope): Type = panic("todo")

  def getTypeOfGroupExpression(value: Expression.GroupExpression, scope: Scope): Type = panic("todo")
  ...
  def inferTypeOfFieldFromVariable(symbol: Symbol, links: SymbolLinks, value: StatementSyntax.VariableDeclarationStatement): Type = {
  ...
  def inferTypeOfLocal(symbol: Symbol, links: SymbolLinks, value: StatementSyntax.VariableDeclarationStatement): Type = panic("todo")
  ...
  def getTypeOfMatchExpression(expression: Expression.MatchExpression, scope: Scope): Type = panic("todo")

  def getTypeOfNewExpression(value: Expression.NewExpression, scope: Scope): Type =
    getTypeOfName(value.name, scope)

  def getTypeOfUnaryExpression(value: Expression.UnaryExpression, scope: Scope): Type =
    getTypeOfExpression(value.expression, scope)

  def getTypeOfUnitExpression(value: Expression.UnitExpression, scope: Scope): Type = panic("todo")

  def getTypeOfWhileExpression(value: Expression.WhileExpression, scope: Scope): Type = panic("todo")
  ...
  def getTypeOfString(name: string, location: TextLocation, scope: Scope): Type = {
    val scalaType = if (location.sourceFile.isScala()) {
      if (name == "String") {
        stringType
      } else if (name == "Int") {
        intType
