...
case class Transpiler(syntaxTrees: Array[SyntaxTree], root: Symbol, outputPath: string) {
  ...
  def transpileVariableDeclarationStatement(get: VariableDeclarationStatementSyntax, context: TranspilerContext): unit = {
  ...
  def transpileMatchExpression(expr: ExpressionSyntax.MatchExpression, context: TranspilerContext): unit = {
  ...
  def transpileArrayCreationExpression(expr: ArrayCreationExpressionSyntax, context: TranspilerContext): unit = panic("todo")

  def transpileBinaryExpression(expr: BinaryExpressionSyntax, context: TranspilerContext): unit = {
  ...
  def transpileGroupExpression(expr: GroupExpressionSyntax, context: TranspilerContext): unit = {
    transpileToken(expr.openParen, context)
    transpileExpression(expr.expression, context)
    transpileToken(expr.closeParen, context)
  }
  ...
  def transpileIndexExpression(expr: IndexExpressionSyntax, context: TranspilerContext): unit = panic("todo")

  def transpileLiteralExpression(expr: LiteralExpressionSyntax, context: TranspilerContext): unit =
  ...
  def transpileNewExpression(expr: NewExpressionSyntax, context: TranspilerContext): unit = {
  ...
  def transpileUnaryExpression(expr: UnaryExpressionSyntax, context: TranspilerContext): unit = {
    transpileToken(expr.operator, context)
    transpileExpression(expr.expression, context)
  }

  def transpileUnitExpression(expr: UnitExpressionSyntax, context: TranspilerContext): unit = {
    transpileToken(expr.openParen, context)
    transpileToken(expr.closeParen, context)
  }

  def transpileWhileExpression(expr: WhileExpressionSyntax, context: TranspilerContext): unit = {
  ...
  def transpileContinueStatement(stmt: ContinueStatementSyntax, context: TranspilerContext): unit = panic("todo")

  def transpileBreakStatement(stmt: BreakStatementSyntax, context: TranspilerContext): unit = panic("todo")
