...
case class Transpiler(syntaxTrees: Array[SyntaxTree], root: Symbol, outputPath: string) {
  ...
  def transpileExpression(expression: Expression, context: TranspilerContext): unit = {
    expression match {
      ...
      case value: Expression.IfExpression => transpileIfExpression(value, context)
  ...
  def transpileIfExpression(expr: Expression.IfExpression, context: TranspilerContext): unit = {
